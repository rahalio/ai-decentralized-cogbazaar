...
const tenantProfilesById = new Map<string, SandboxTenantProfile>();
...
export function getOrCreateTenantProfile(tenantId: string): SandboxTenantProfile {
  ...
  const profile: SandboxTenantProfile = {
    tenantId,
    displayNameEn: tenantId === 'tnt_demo' ? 'Cogbazaar Demo' : tenantId,
    displayNameAr: tenantId === 'tnt_demo' ? 'كوجبازار تجريبي' : tenantId,
  };
...
export function generateApiKeySecret(prefix = 'cogbazaar_demo'): string {
