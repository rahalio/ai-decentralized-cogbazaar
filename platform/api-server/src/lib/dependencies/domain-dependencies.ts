...
const DOMAIN_PATH_PATTERNS: Array<{ pattern: RegExp; domain: string }> = [
  ...
  { pattern: /\/grant-pools\//, domain: "grant-pools" },
  { pattern: /\/identity\//, domain: "identity" },
  { pattern: /\/jobs\//, domain: "jobs" },
  { pattern: /\/listings\//, domain: "listings" },
  { pattern: /\/pocw-attestations\//, domain: "pocw-attestations" },
  { pattern: /\/policy-rules\//, domain: "policy-rules" },
  { pattern: /\/royalty-shares\//, domain: "royalty-shares" },
];
...
export async function injectDomainDependencies(
  ...
  if (!domainDeps) {
    ...
    switch (domain) {
      case "grant-pools":
        domainDeps = buildGrantPoolsDomainModule(dynamoClient);
        break;
      case "identity":
      ...
      case "jobs":
        ...
        break;
      case "listings":
        domainDeps = buildListingsDomainModule(dynamoClient);
        break;
      case "pocw-attestations":
        domainDeps = buildPocwAttestationsDomainModule(dynamoClient);
        break;
      case "policy-rules":
        domainDeps = buildPolicyRulesDomainModule(dynamoClient);
        break;
      case "royalty-shares":
        domainDeps = buildRoyaltySharesDomainModule(dynamoClient);
        break;
      default:
        return;
    }
...
export interface DomainDependencies {
  "grant-pools": GrantPoolsDomainModule;
  identity: IdentityDomainModule;
  jobs: JobsDomainModule;
  listings: ListingsDomainModule;
  "pocw-attestations": PocwAttestationsDomainModule;
  "policy-rules": PolicyRulesDomainModule;
  "royalty-shares": RoyaltySharesDomainModule;
}
 ...
 */
export function getGrantPoolsDeps(
  request: FastifyRequest,
): DomainDependencies["grant-pools"] {
  return getDomainDeps(request, "grant-pools");
}
...
export function getListingsDeps(
  request: FastifyRequest,
): DomainDependencies["listings"] {
  return getDomainDeps(request, "listings");
}
export function getPocwAttestationsDeps(
  request: FastifyRequest,
): DomainDependencies["pocw-attestations"] {
  return getDomainDeps(request, "pocw-attestations");
}
export function getPolicyRulesDeps(
  request: FastifyRequest,
): DomainDependencies["policy-rules"] {
  return getDomainDeps(request, "policy-rules");
}
export function getRoyaltySharesDeps(
  request: FastifyRequest,
): DomainDependencies["royalty-shares"] {
  return getDomainDeps(request, "royalty-shares");
}
